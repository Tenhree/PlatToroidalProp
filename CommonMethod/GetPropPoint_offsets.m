...
for i=1:length(l)
    R=D/2;
    beta=atan(PdivD(i)/rdivR(i)/pi);
    c1=bdivD(i)*D/2-rdivR(i)*R*thets(i)/cos(beta)/cos(alpha(i));
    s=xofsection*bdivD(i)*D;
    ybs=fdivb(i)*bdivD(i)*D*fdivfmax+(tdivb(i)*bdivD(i)*D*tdivtmax/2);
    xb(i,:)=l(i)*totalL+xl(i)+(-c1+s)*cos(alpha(i))*cos(beta)-(ybs*cos(gamma(i)))*sin(beta);
    ...
    yfs=fdivb(i)*bdivD(i)*D*fdivfmax-tdivb(i)*bdivD(i)*D*tdivtmax/2;
    xf(i,:)=l(i)*totalL+xl(i)+(-c1+s)*cos(alpha(i))*cos(beta)-(yfs*cos(gamma(i)))*sin(beta);
